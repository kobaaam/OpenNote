	...
	public function subFolder(IModel $model, $id){
		...
		//get all the folders                            
			...
			foreach($folders as $row){
                            ...
                            $list = $list."<br/>";
                            echo self::boxFactory($row["id"], "folder green", $row["name"], $list,"Folder");
                        }
		//get all the notes
			$folders = $model->getNotesInFolder($id);
			foreach($folders as $row){
				echo self::boxFactory($row["id"], "note", $row["title"], strip_tags(html_entity_decode($row["note"])),null,"Note", $id);
                        }
	}
